...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lio="http://slothsoft.net" xmlns:func="http://exslt.org/functions" extension-element-prefixes="func">
        ...
        <table id="{$id}">
            ...
            <tfoot>
                <tr>
                    ...
                    <th>
                        <xsl:choose>
                            <xsl:when test="lio:final-price()">
                                <xsl:if test="lio:price(part)">
                                    <s>
                                        <xsl:value-of select="format-number(lio:price(part), '0.00')" />
                                    </s>
                                    <xsl:text> </xsl:text>
                                </xsl:if>
                                <xsl:value-of select="format-number(lio:final-price(. | part), '0.00')" />
                                <xsl:text>&#160;€</xsl:text>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="format-number(lio:price(part), '0.00')" />
