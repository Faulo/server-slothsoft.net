...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sfm="http://schema.slothsoft.net/farah/module">
    ...
    <xsl:template name="melnics.translator">
        <script type="module" async="async"><![CDATA[
import Bootstrap from "/slothsoft@farah/js/Bootstrap";
import MelnicsTranslator from "/slothsoft@slothsoft.net/talesof/js/MelnicsTranslator";
        ...
        <div class="Translator MelnicsTranslator">
            ...
            <article>
                ...
                <article>
                    ...
                    <p>
                        <xsl:text>Also, the Melnics sequence </xsl:text>
                        <q>ne</q>
                        <xsl:text> must be written as </xsl:text>
                        <q>ne</q>
                        <xsl:text> when they mean </xsl:text>
                        <q>ネ</q>
                        <xsl:text> and as </xsl:text>
                        <q>n'e</q>
                        <xsl:text> when they mean </xsl:text>
                        <q>ンエ</q>
                        <xsl:text>.</xsl:text>
            ...
            <article>
                ...
                <article>
                    <h3>[1.0.5] - 2025-11-29</h3>
                    <p>
                        <xsl:text>More bugfixes! This time it should really work in all browsers, for sure. 🥲</xsl:text>
                    </p>
                </article>
                <article>
                        ...
                        <dl>
                            <xsl:for-each select="kana/kanji">
                                <dt xml:lang="ja-jp">
                                    ...
                                    <!-- <xsl:call-template name="word.complete"> <xsl:with-param name="lang" select="'ja-jp'" /> <xsl:with-param name="audio" select="$player" /> <xsl:with-param name="base" select="$base" /> <xsl:with-param name="top" select="$top" /> </xsl:call-template> -->
                                    <!-- <xsl:variable name="player"> <xsl:choose> <xsl:when test="string-length(@name) = 0"> <xsl:value-of select="../@player-uri"/> </xsl:when> <xsl:otherwise> <xsl:value-of select="@player-uri"/> </xsl:otherwise> </xsl:choose> </xsl:variable> <xsl:call-template name="word.audioPlayer"> 
                                        <xsl:with-param name="uri" select="$player"/> </xsl:call-template> <xsl:value-of select="@name"/> <xsl:if test="string-length(@name) = 0"> <xsl:value-of select="../@name"/> </xsl:if> -->
                                </dt>
                                <dd>
