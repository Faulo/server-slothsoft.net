...
<xsl:stylesheet version="1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/*">
		<div>
			<xsl:apply-templates select="*[@name = 'kana.test']/resource" />
		</div>
	</xsl:template>
...
addEventListener(
	...
	false
			...
			<xsl:for-each select="data">
				...
				<!-- <label> <input type="text" readonly="readonly" value="{$testKana/@text}" class="kanaStrokeOrder"/> </label> <label> <input type="text" readonly="readonly" value="{$testKana/@text}" class=""/> 
					</label> -->

				<label>
					<span>
						Leave one whitespace between each transcription
						<small>(e.g. "a ka sa ta")</small>
						:
					</span>
				...
				<div class="floatLeft">
					<label>
						Selected:
						<xsl:value-of select="count(kana[@checked])" />
						/
						<xsl:value-of select="count(kana)" />
					</label>
					<label>
